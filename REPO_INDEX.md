...
原則:
- 共通前提や再利用する定型は情報源に置く
- repo 固有の今回だけの状況や残件は、その repo の `CLAUDE.md` に置く
...
各 repo では、共通前提だけでは足りない「今回の作業条件」を `CLAUDE.md` に置く。  
`CLAUDE.md` は、毎回なんでも自動更新するメモではなく、節目で Claude Code が更新する現場メモとして扱う。  
そこには最低限、次を入れる。
...
使い分け:
- 情報源 = 変わりにくい土台
- `CLAUDE.md` = 今回だけの具体指示と、次回再開用の現場メモ
- スレッド = 単発ログ、途中経過、試行錯誤
