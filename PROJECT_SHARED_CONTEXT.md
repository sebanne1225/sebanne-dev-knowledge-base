...
### Codex に直接頼んでよい軽作業
- Windows 環境で「エクスプローラーでコピーした状態にして」と依頼された場合、Codex は対象ファイルを Explorer の貼り付けで使えるファイルコピー形式でクリップボードへ入れてよい
- この依頼は、単なるテキストコピーではなく、貼り付け先フォルダで `Ctrl+V` できる「ファイルのコピー状態」を指すものとして扱う
- この時、複数ファイルが対象なら 1 つにまとめた仮ファイルへ変換せず、各ファイルを別々のファイルとしてコピー状態にする
- 対象は、明示されたファイル群があればそれを優先し、なければ直前に扱っている repo の変更ファイル一覧を使ってよい
- 「えくこぴ」は「エクスプローラーでコピーした状態にして」の短縮呼び名として扱ってよい
- 実行後は、どの repo の何件をコピー状態にしたかだけ短く返せばよい
...
- Codex に渡す依頼文では、毎回 `CODEX_HANDOFF.md` の扱いを明示し、`before / after / none` を基本指定にし、必要なら `full / minimal` も添える
...
- スレ終了時は `THREAD_CLOSEOUT_TEMPLATE.md` を使い、まず「どこに残すか」を分けてから短く整理する
