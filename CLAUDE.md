...
- Phase 1 の網羅的実装確認は省略しない。closeout まとめの精度に直結する
...
- `AssetDatabase.CreateAsset` は既存パスにアセットがあると上書きしない
  ...
  → Texture2DArray / AnimationClip: インプレース更新 API がないため 完全削除 → `AssetDatabase.Refresh()` → `CreateAsset`（GUID は変わる）
  → PNG テクスチャ: `File.WriteAllBytes` で自然に上書き（.meta 保持で GUID 不変、対応不要）
  → AnimatorController: 完全削除 → `Refresh` → `CreateAnimatorControllerAtPath`（既存があると空で返るため事前削除必須）
- `AudioSource.m_Enabled` の OFF→ON では `playOnAwake` は発火しない。
  `m_Enabled` はコンポーネント有効化のみ。音声リスタートには `GameObject.m_IsActive` の OFF→ON が必要
- Audio の `m_IsActive` を一部ステートだけで操作すると、WD=true の他ステートで
  デフォルト値（Prefab 初期値）に書き戻される。全ステートで明示キーフレームを持つ（WD 非依存設計の延長）
- FX 制御（EX メニュー等）と Animator 停止（keepAnimatorState）は構造的に排他。
  MergeAnimator で FX 統合すると Animator は常時動作し止められない。
  「オフ中に止めて途中から再開」は VRChat + MergeAnimator 環境では実現困難
- `AssetDatabase.DeleteAsset` は内部的に OS のゴミ箱にファイルを送る（Unity 仕様）
  → 生成物の上書き削除で使うとゴミ箱にファイルが蓄積する
  → `File.Delete(fullPath)` + `File.Delete(fullPath + ".meta")` + `AssetDatabase.Refresh()` で完全削除にする
  → フォルダ削除は `Directory.Delete(fullPath, true)` + `.meta` 削除 + `Refresh`
  → 共通ヘルパー（DeleteFileAndMeta / DeleteFolderAndMeta）にまとめると安全
- NDMF Generating フェーズでは AnimatorController はクローン済み
  → AssetDatabase 操作不要、SetDirty/SaveAssets 不要。直接メモリ上で操作する
  → ソース Controller はユーザーのオリジナルアセット（読み取りのみ）
- AnimatorStateMachine.RemoveStateMachine() で SubStateMachine を丸ごと削除可能
  → ただし AnyState Transition は自動削除されない → 手動で配列フィルタ再代入が必要
- AnimatorStateTransition の遷移先は destinationState / destinationStateMachine / isExit の 3 パターン
  → destinationState == null でスキップすると SubStateMachine への遷移を見落とす
- StateMachineBehaviour は Object.Instantiate() でクローン可能（NDMF クローン上ではアセット登録不要）
- VRChat の AFK は Action Layer（デフォルトウェイト 0）で動作
  → AFK ステートの「骨格」（Prepare AFK / BlendOut AFK / Restore Tracking AFK）と
    「中身」（SubStateMachine 内の Intro/AFK/Loop/Outro）を分離して扱うのが安全
- `AnimatorStateMachine.states` / `.stateMachines` はコピーを返す（`controller.layers` と同様）
  → 要素の追加・削除後は配列ごと再代入が必要
- `RemoveState()` は対象ステートを所有する親 StateMachine で呼ぶ必要がある
  → rootStateMachine で SubSM 内のステートを RemoveState() しても削除されない
- AnyState Transition は StateMachine ごとに持つ
  → AFK 遷移を網羅するには rootSM だけでなく SubSM も再帰的に走査する
- AnyState Transition の canSelf=False は「現在のステートが遷移先と同じ時」のみ再発火を防ぐ
  → AFK_Intro → AFK に遷移した後、AnyState → AFK_Intro が再発火する（現在ステートが AFK なので canSelf はブロックしない）
  → AFK 入口には AnyState ではなく、ターゲットの元の入口遷移を付け替える方式を使う
- VRC StateBehaviour（VRCPlayableLayerControl / VRCAnimatorTrackingControl）は ScriptableObject.CreateInstance<T>() で生成可能
  → NDMF クローン上では AssetDatabase 登録不要
  → asmdef に VRC.SDK3A 参照があれば using で型アクセス可能（リフレクション不要）
  → VRCPlayableLayerControl: .layer (BlendableLayer enum), .goalWeight (float), .blendDuration (float)
  → VRCAnimatorTrackingControl: .trackingHead/.trackingLeftHand 等 (TrackingType enum: Tracking/Animation/NoChange)
  → 基底型は VRC.SDKBase の VRC_PlayableLayerControl / VRC_AnimatorTrackingControl
- NDMF プラグインの Runtime コンポーネントには `VRC.SDKBase.IEditorOnly` を実装する（マーカーインターフェース、メソッド不要）
  → これがないと VRChat SDK の validation で「will be removed by the client」警告が出る
  → `IEditorOnly` は SDK validation スキップ、`Object.DestroyImmediate` はクローンからの実際の除去。役割が違うので両方必要
  → DestroyImmediate は Generating フェーズの処理を try-finally で囲み、finally で実行する（Audio Safety Guard / AFK Changer で共通パターン）
- VCC は同一 version の zip を再ダウンロードしない。公開済み version の中身を変えた場合は patch version を上げる
