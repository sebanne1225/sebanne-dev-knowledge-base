...
#### VSCode 開きっぱなし対策
Claude Code にファイルを編集させる前に、VSCode で開かれている
ファイルがあれば全て閉じる。編集後に再度開く必要はない。

理由: VSCode で開いている状態でファイルを編集するとバッファが dirty になり、
ユーザーが手動保存した時に Claude Code の変更が失われるリスクがあるため。
...
Claude Code のビルトインルール（不要な機能追加・抽象化・コメント追加の禁止等）に加え、
このプロジェクト固有のルールを以下に定める。ビルトインと重複する項目は省略している。

**判定基準:**
変更した全ての行が、依頼内容に直接たどれること。
たどれない変更が含まれていたら、その行を元に戻す。

**死にコードの扱い（ビルトインの上書き）:**
元からあった死にコードはこのプロジェクトでは報告だけして消さない。
自分の変更で不要になった import / 変数 / 関数だけ消す。
ビルトインの「unused なら delete completely」はこのプロジェクトでは適用しない。

**連鎖変更の扱い:**
「直接関係する」は、依頼内容を成立させるために必要な全ファイル・全行を含む。
NDMF プラグインの Component → Editor → Plugin 等の連鎖は「直接関係する変更」。

**対象外:**
事前に plan で承認されたリファクタ・構造変更・namespace 変更等はこの原則の対象外。
plan の範囲内で必要な変更は全て行う。
