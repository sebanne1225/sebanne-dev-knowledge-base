...
2. Claude が Notion セッション記録 DB にセッション記録を投入する（やったこと・設計変更・後回し経緯を含む）
3. セッション記録の中から `CLAUDE.md` に昇格できそうなものがあれば寄せる
4. 次フェーズ候補は Notion DB が正本。スレ中に出た「後回し」は Notion 次フェーズ候補 DB に直接追加する。closeout 時に追加漏れがないか確認する
...
## 11. リリース前走査の推奨フロー
- 機能が固まったら棚卸し（コード面の網羅確認。観点はセクション 7 の closeout チェックリストに準ずる）
